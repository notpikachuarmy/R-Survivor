...
function renderDevSavePanel() {
  ...
  devSaveContent.innerHTML = `
    <section class="dev-save-section">
      <h3>Desbloqueos</h3>
      <div class="dev-save-grid">
        ${unlockKeys.map(key => `
          <label><input type="checkbox" data-dev-unlock="${key}" ${saveData.unlocks[key] ? "checked" : ""}> ${key}</label>
        `).join("")}
      </div>
    </section>
    <section class="dev-save-section">
      <h3>Valores rápidos</h3>
      <label>Récord <input type="number" data-dev-number="bestScore" value="${saveData.bestScore || 0}"></label>
      <label>Mejor tiempo <input type="number" data-dev-number="bestTime" value="${Math.round(saveData.bestTime || 0)}"></label>
      <label>Monedas <input type="number" data-dev-number="coins" value="${saveData.coins || 0}"></label>
    </section>
    <section class="dev-save-section">
      <h3>Stats</h3>
      <div class="dev-save-grid">
        ${statKeys.map(key => `
          <label>${key}<input type="number" data-dev-stat="${key}" value="${saveData.stats[key] || 0}"></label>
        `).join("")}
      </div>
    </section>
    <div class="dev-save-actions">
      <button id="devSaveApplyButton">Guardar cambios</button>
      <button id="devSaveUnlockAllButton">Desbloquear todo</button>
  ...
  document.getElementById("devSaveApplyButton")?.addEventListener("click", applyDevSaveChanges);
  document.getElementById("devSaveUnlockAllButton")?.addEventListener("click", () => {
